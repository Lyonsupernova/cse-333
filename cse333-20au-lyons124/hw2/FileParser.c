  ...
  while (left_to_read > 0) {
    ...
    if (result == -1) {
      if (errno != EINTR && errno != EAGAIN) {
        free(buf);
        perror("Write Failed: unrecoverable errors");
        return NULL;
      }
    } else if (result == 0) {
      left_to_read = 0;
...
static void InsertContent(HashTable *tab, char *content) {
  ...
  while (1) {
    // check if current pointer points to a null-terminator
    if (*curptr == '\0') {
      break;
    }
    // check if the current pointer is alphabetic character
    // if alphabetic, then turned to lowrer case
    // if not, overwrite with null terminator
    // and add to hashtable if it points to a word
    if (isalpha(*curptr) != 0) {
      *curptr = tolower(*curptr);
    } else {
      *curptr = '\0';
      if (isalpha(*(curptr - 1)) != 0 && curptr != content) {
...
static void AddWordPosition(HashTable *tab, char *word,
  ...
  if (HashTable_Find(tab, hashKey, &kv)) {
    ...
    Verify333(wp != NULL);
    // allocate space for string, add one more byte for null terminator
    char* newString = (char*) malloc(strlen(word) + 1);
    Verify333(newString != NULL);
    // copy the word to new string
    strncpy(newString, word, strlen(word) + 1);
    // allocate linkedlist for the word position
