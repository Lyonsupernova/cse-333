...
void MemIndex_AddPostingList(MemIndex *index, char *word, DocID_t docid,
  ...
  HashTable_Insert(wp->postings, mi_kv, &postings_kv);
}
...
LinkedList* MemIndex_Search(MemIndex *index, char *query[], int querylen) {
  ...
  if (HashTable_Find(index, key, &kv)) {
    wp = kv.value;
    it = HTIterator_Allocate(wp->postings);
    Verify333(it != NULL);
    // find the word in the documents 
    // and restore the number of appearing frenqeuncy
  ...
  for (i = 1; i < querylen; i++) {
    ...
    for (j = 0; j < numdocs; j++) {
      ...
      Verify333(sr != NULL);
      // find the doc and update the rank
