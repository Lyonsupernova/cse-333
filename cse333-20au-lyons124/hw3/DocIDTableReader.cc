...
bool DocIDTableReader::LookupDocID(const DocID_t &docID,
  ...
  for (auto it = elements.begin(); it != elements.end(); it++) {
    ...
    Verify333(fread(&currHeader, sizeof(currHeader), 1, file_) == 1);
    ...
    if (currHeader.docID == docID) {
      ...
      list<DocPositionOffset_t> dpo_list;
      DocIDElementPosition dep;
      for (int i = 0; i < currHeader.numPositions; i++) {
        Verify333(fread(&dep, sizeof(dep), 1, file_) == 1);
        dep.toHostFormat();
        dpo_list.push_back(dep.position);
      }
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() {
  ...
  for (int i = 0; i < header_.numBuckets; i++) {
    ...
    Verify333(fread(&bucketRec, sizeof(bucketRec), 1, file_) == 1);
    ...
    for (int j = 0; j < bucketRec.chainNumElements; j++) {
      ...
      Verify333(fread(&elementPos, sizeof(elementPos), 1, file_) == 1);
