...
int WriteIndex(MemIndex *mi, DocTable *dt, const char *filename) {
  ...
  int headbytes = WriteHeader(f, dtbytes, membytes);
  if (headbytes == kFailedWrite) {
    fclose(f);
    unlink(filename);
    return kFailedWrite;
  }
...
static int WriteHeader(FILE *f,
  ...
  for (int i = 0; i < doctableBytes + memidxBytes; i++) {
    ...
    crc.FoldByteIntoCRC((u_int8_t) buffer);
  }
...
static int WriteHashTable(FILE *f,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int32_t numElts = LinkedList_NumElements(ll); 
    int recordBytes;
    int bucketBytes;
    recordBytes = WriteHTBucketRecord(f, recordPos, numElts, bucketPos);
    recordPos += recordBytes;
    if (numElts != 0) {
      bucketBytes = WriteHTBucket(f, bucketPos, ll, fn);
      bucketPos += bucketBytes;
    }
  }
...
static int WriteHTBucketRecord(FILE *f,
  ...
  BucketRecord br;
  br.position = bucketOffset;
  br.chainNumElements = numElts;
  br.toDiskFormat();
...
static int WriteHTBucket(FILE *f,
  ...
  for (int i = 0; i < numElts; i++) {
    ...
    ElementPositionRecord epr(elementPos);
    epr.toDiskFormat();
    ...
    elementPos += len;  // you probably want to change this
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocidToDocnameFn(FILE *f,
  ...
  uint16_t filenameBytes = strlen(filename);
  ...
  if (fwrite(filename, 1, filenameBytes, f) != filenameBytes) {
    return kFailedWrite;
  }

  // STEP 11.
  // calculate and return the total amount written.
  return sizeof(header) + filenameBytes;  // you may want to change this
}
...
static int WriteDocidToPositionListFn(FILE *f,
  ...
  return sizeof(header) + numPositions * sizeof(position);  // you may want to change this
}
...
static int WriteWordToPostingsFn(FILE *f,
  ...
  uint16_t wordBytes = static_cast<uint16_t>(strlen(wp->word));
  ...
  if (fwrite((wp->word), wordBytes, 1, f) != 1) {
    return kFailedWrite;
  }
  // STEP 19.
  // Calculate and return the total amount of data written.
  return htBytes + wordBytes + sizeof(header);  // you may want to change this
}
}  // namespace hw3
