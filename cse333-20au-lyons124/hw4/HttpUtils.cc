...
bool IsPathSafe(const string &rootdir, const string &testfile) {
  ...
  // STEP 1
  char root_dir_buffer[PATH_MAX];
  char test_file_buffer[PATH_MAX];
  // return the canoicalized absolute path name to the pointer
  // resolves the references to "/./" or "/../" and extra '/'
  if (!realpath(rootdir.c_str(), root_dir_buffer) ||
    !realpath(testfile.c_str(), test_file_buffer)) {
    return false;
  }
  if (strncmp(root_dir_buffer,
              test_file_buffer,
